...
@section('page-title') Избранное @endsection

@section('content')

<div>
    <h1 class="text-xl font-bold my-4">Избранное</h1>
    <div class="">
        @if(count($products) > 0)
                ...
                <div  class="flex flex-col justify-center items-center text-center border-4 rounded-xl p-4 cursor-pointer my-4 snap-center">
                        <div class="flex flex-col items-center">
                            ...
                            <div class="flex space-x-4 items-center">
                                <h1 class="font-bold text-green-700 my-2 text-xl">${{ $product->price }}</h1>
                                @if($product->prev_price > $product->price)
                                <s class="">
                                    <h1 class=" font-bold  my-2">${{ $product->prev_price }}</h1>
                                </s>
                                @endif
                            </div>
                        </div>
            ...
            @endforeach
        @else 
            <h1>Пока пусто. Вам пора что-то приметить...</h1>
        @endif
    </div>
</div>
